...
export default function FetchTab() {
  ...
  const { data: fetchResult, isLoading: isFetching, refetch } = useQuery({
    queryKey: ['efactura-anaf-messages', selectedCif, days, filter, page],
    queryFn: () =>
      efacturaApi.fetchMessages({ cif: selectedCif, days, page, filter }),
    enabled: !!selectedCif,
  })

  const importMut = useMutation({
    mutationFn: () =>
      efacturaApi.importFromAnaf(selectedCif, Array.from(selectedIds)),
    onSuccess: () => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="space-y-1">
          ...
          <Select value={selectedCif} onValueChange={(v) => { setSelectedCif(v); setPage(1); setSelectedIds(new Set()) }}>
        ...
        <Button onClick={() => refetch()} disabled={!selectedCif || isFetching}>
      ...
      {!selectedCif ? (
        <EmptyState
          ...
          description="Choose a company to fetch messages from ANAF"
        />
      ) : messages.length === 0 && !isFetching ? (
        <EmptyState
          icon={<FileText className="h-10 w-10" />}
          title="No messages"
          description={`No messages found for CIF ${selectedCif} in the last ${days} days`}
        />
      ) : (
                ...
                <tr key={msg.id} className="border-b hover:bg-muted/30">
                  ...
                  <td className="p-2">
                    <div className="flex gap-1">
                      ...
                      >
                        <a
                          href={efacturaApi.downloadMessageUrl(msg.id, selectedCif)}
                          download
                        >
                      ...
                      >
                        <a
                          href={efacturaApi.exportPdfUrl(msg.id, selectedCif)}
                          target="_blank"
